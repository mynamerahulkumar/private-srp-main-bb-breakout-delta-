...
class BollingerReversalStrategy:
    ...
    def evaluate(self, candles: list[Candle], indicators: IndicatorSnapshot) -> SignalDecision:
        ...
        sell = SignalDecision(
            ...
            rsi_valid=(not rsi_config.get("rsi_enabled", True))
            or indicators.current_rsi >= float(rsi_config["rsi_sell_level"]),
        ...
        buy = SignalDecision(
            ...
            rsi_valid=(not rsi_config.get("rsi_enabled", True))
            or indicators.current_rsi <= float(rsi_config["rsi_buy_level"]),
            confirmation_active=True,
        )
